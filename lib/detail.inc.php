...
class detail
{
    ...
    public function getItemCopy() {
      ...
      $_output .= '<tr style="background:grey;text-align:center;color:white;font-weight:bold;"><td>じょうたい</td><td>ISBN/ISSN</td></tr>';
      foreach ($copies as $copy_d) {
        ...
        if ($loan_stat_q->num_rows > 0) {
            ...
            $_output .= '<span class="label label-important status-on-loan">かしだしちゅう</span>'; //mfc
        } else if ($copy_d['no_loan']) {
            $_output .= '<span class="label label-important status-not-loan">かしだしちゅう'.' - '.$copy_d['item_status_name'].'</span>';
        } else {
            $_output .= '<span class="label label-info status-available">かりられます'.(trim($copy_d['item_status_name'])?' - '.$copy_d['item_status_name']:'').'</span>';
        }
