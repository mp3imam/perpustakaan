...
if (isset($_GET['search']) && !empty($_GET['search'])) {
  ...
  if ($is_adv) {
    $search_result_info .= '= キーワード：  <strong>{biblio_list->num_rows}</strong> けんあります<strong class="search-found-info-keywords">'.$keywords_info.'</strong>  ';
  ...
  if (isset($_GET['page']) AND $_GET['page'] > 1) {
    ...
    $msg = str_replace('{page}', $page, __('<strong>{page}</strong>ページ目 （ 全て ： <strong>{total_pages}</strong> ページ）')); //mfc
  ...
  // $search_result_info .= '<div class="search-query-time">さがしおわるまでのじかん： <b>'.$biblio_list->query_time.'</b></div>';
