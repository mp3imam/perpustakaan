...
class member
{
    ...
    public static function getOverduedLoan($obj_db, $str_member_id)
    {
        ...
        $_ovd_title_q = $obj_db->query(sprintf('SELECT l.item_code, i.price, i.price_currency,
            ...
            WHERE (l.is_lent=1 AND l.is_return=0 AND TO_DAYS(due_date) < TO_DAYS(\''.date('Y-m-d', strtotime('+ 2 days')).'\')) AND l.member_id=\'%s\'', $str_member_id));
    ...
    public function sendOverdueNotice()
    {
        ...
        $_overdue_data .= '<tr><th>' . __('Title') . '</th><th>' . __('ISBN Code') . '</th><th>' . __('Loan Date') . '</th><th>' . __('Due Date') . '</th></tr>'."\n";
        $_arr_overdued = self::getOverduedLoan($this->obj_db, $this->member_id);
        foreach ($_arr_overdued as $_overdue) {
            $_overdue_data .= '<tr>';
            $_overdue_data .= '<td>'.$_overdue['title'].'</td><td>'.$_overdue['item_code'].'</td><td>'.$_overdue['loan_date'].'</td><td>'.$_overdue['due_date'].'</td>'."\n";
            $_overdue_data .= '</tr>';
        }
        $_overdue_data .= '</table>';
        ...
        $_mail->Subject = str_replace(array('{member_name}', '{member_email}'), array($this->member_name, $this->member_email), __('Gentle Reminder of JJC Library Book Loan [JJCより] 図書の返却予定日のお知らせ'));
        ...
        $_sent = $_mail->Send();
        // return $_sent;
        // $iGetTo      = $this->member_email;
        // $iGetSubject = str_replace(array('{member_name}', '{member_email}'), array($this->member_name, $this->member_email), __('Overdue Notice for Member {member_name} ({member_email})'));
        // $iGetMessage = $_mail->MsgHTML($_message);
        // $iGetHeaders = 'From: ' . $sysconf['mail']['from'] . "\r\n" .
        //     'Reply-To: ' . $sysconf['mail']['from'] . "\r\n" .
        //     'X-Mailer: PHP/' . phpversion();
        // $_sent = mail($iGetTo, $iGetSubject, $iGetMessage, $iGetHeaders);
        if (!$_sent) {
