...
class report_datagrid extends simbio_datagrid
{
    ...
    protected function makeOutput($int_num2show = 30)
    {
        ...
        $strButtonSendReminder = '';
        if($_SERVER['PHP_SELF'] == '/jjc-dev/admin/modules/reporting/customs/overdued_list.php'){
            $strButtonSendReminder = '<a class="printReport usingAJAX" href="'.MWB.'membership/overdue_mail.php'.'" postdata="memberID=sendEmailToAllOverdue" loadcontainer="sendEmailToAllStatus">Send Reminder To All</a><div id="sendEmailToAllStatus"></div>';
        }
        $_buffer .= '<div class="printPageInfo"><strong>'.$this->num_rows.'</strong> '.__('record(s) found. Currently displaying page').' '.$this->current_page.' ('.$int_num2show.' '.__('record each page').') <a class="printReport" onclick="window.print()" href="#">'.__('Print Current Page').'</a>'.$strButtonSendReminder.'</div>'."\n"; //mfc
