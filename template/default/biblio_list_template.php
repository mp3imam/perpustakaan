...
function biblio_list_format($dbs, $biblio_detail, $n, $settings = array(), &$return_back = array()) {
  ...
  $title_link = '<a href="'.$detail_url.'" class="titleField" itemprop="name" property="name" title="しょうさいひょうじ">'.$title.'</a>';
  ...
  // button
  $xml_button = '';
  $detail_button = '<a href="'.$detail_url.'" class="detailLink" title="'.__('しょうさいをひょうじするためには本・DVDのなまえをクリックしてください').'">'.__('本のしょうさい</br></br>').'</a>';
  if ($settings['xml_detail']) {
    $xml_button = '<a href="'.$detail_url.'&inXML=true" class="xmlDetailLink" title="'.__('しょうさいをひょうじするためには本・DVDのなまえをクリックしてください').'" target="_blank">'.__('本のしょうさい').'</a>';
  }
  ...
  $output .= '<div class="" itemscope itemtype="http://schema.org/Book" vocab="http://schema.org/" typeof="Book"><div class="cover-list">'.$image_cover.'</div>';
