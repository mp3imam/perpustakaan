...
if (isset($_POST['tempLoanID'])) {
    ...
    if ($add == LOAN_LIMIT_REACHED) {
        ...
        if ($sysconf['loan_limit_override']) {
            ...
            echo '} else { self.location.href = \'index.php\';}';
            echo '</script>';
        } else {
            echo '<script type="text/javascript">';
        ...
        echo '<script type="text/javascript">';
        echo 'alert(\''.__('かりられません。 メンバーカードが使えません。').'\');';
        echo 'location.href = \'index.php#\';';
        echo '</script>';
        ...
        // $save_data_session = base64_encode(serialize($_SESSION));
        // echo '<script>$("#tempSessionLoan").val('.$save_data_session.');</script>';
    }
    // exit();
}
...
if (isset($_GET['reserveAlert']) AND !empty($_GET['reserveAlert'])) {
    ...
    $reserve_q = $dbs->query(sprintf('SELECT r.member_id, m.member_name
        ...
        WHERE item_code=\'%s\' ORDER BY reserve_date desc LIMIT 1', $reservedItem));
